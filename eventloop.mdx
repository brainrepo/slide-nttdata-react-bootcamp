...
The Event Loop è uno dei piu importanti aspetti di javascript
...
🚨 L'unica attenzione necessaria quando si scrive codice javascript è che:
...
### -- non può interagire con la pagina --
...
Questo è il motivo percui in javascript dobbiamo usare spesso le callback
...
il mondo in cui scegliamo di gestire i processi asincroni puo' fare la differenza di sostenibiltà della nostra app
...
Ogni funzione prende come argomento un altra funzione che è chiamata con un parametro che è la risposta della precedente funzione.
...
Per semplificare il nostro codice e renderlo più leggibile, 

riducendo le linee di codice e quindi limitando l'insorgere di bug 🐛
...
Una promessa di un valore che verrà restituito nel futuro, promessa che può avere 3 stati.
...
Una promise è risolta quando il 
...
Una promise è rigettata quando l’attività asincrona associata 

**non restituisce un valore** 

o perché si è **verificata un’eccezione**

oppure perché il valore restituito **non è considerato valido**
...
La richiesta di esecuzione di un’attività asincrona 

**è partita ma non abbiamo ancora ricevuto un risultato**
...
In questo modo se per un qualsiasi motivo la promise dovesse essere rigettata verrà chiamata la funzione dentro catch()
...
Reactive programming è un paradigma di programmazione dichiarativa
...
il valore di a$ sarà sempre 30<br/>
...
Durante il cambiamento di b e c, a verrà notificata e eseguirà nuovamente il calcolo
...
RxJs è una libreria per trasformare, comporre e ricercare <br />
...
Il vero carico cognitivo è dato dal **pensare in modo reattivo**

Un modo per descriverlo è

> _Think of RxJS as “LoDash” for handling asynchronous events_.
...
Un Observable è **winebox**, infatti un observable non è altro 
...
Se l'abbonamento è in corso scalo una mensilità e notifico l'obeserver (l'abbonato al servizio) con una nuova cassetta di vini
...
Per avere un abbonato è necessario realizzare una **Subscription**
...
L'observer sono io che mi sto abbonando e qui definirò il mio comportamento
...
const subscription = observable.subscribe({
  next: x => console.log(`${x} e io me lo bevo 🍷`),
  error: err => console.error("something wrong occurred: " + err),
  complete: () => console.log("📭 abbonamento terminato")
});
...
```js title="modalità viking / procedural"
...
```js  title="modalità functional"

import { timer } from "rxjs";
import { map, take } from "rxjs/operators";

const randWine = mesi => {
  return `📦 Il vino per il ${mesi} è ${
...
subject.subscribe({
  next: x => console.log(`${x} e Mario se la beve 🍷`)
});
subject.subscribe({
  next: x => console.log(`${x} e Lucia se la beve 🍷`)
});
